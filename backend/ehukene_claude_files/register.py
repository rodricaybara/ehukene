...
async def register_device(
    ...
    # 1. Verificar que el hostname no exista ya
    existing = (
    ...
    db.add(device)
    await db.flush()  # obtener el UUID generado antes del commit

    return DeviceRegisterResponse(
        device_id=device.id,
        hostname=device.hostname,
        api_key=api_key_plain,  # única vez que se devuelve en claro
        created_at=now,
    )
